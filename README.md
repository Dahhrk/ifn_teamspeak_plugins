...
| `sticky-tools` | Standalone jail tool: right-click a user to send them to the jail channel with the Sticky group applied (indefinite or 15/60 min). Timed jails auto-release - moves them back to their original channel and removes Sticky (group removal works offline via database ID). Global "Sticky board" lists jailed users and time remaining. |
